...
2. search_keywords: 의약품 데이터베이스 검색에 사용할 키워드 리스트
   - 약 종류, 증상, 성분명, 약효 분류만 포함 (대상자 조건인 "임산부", "어르신", "소아" 등은 제외)
   ...
   - 예: "임산부가 먹을 수 있는 감기약?" → ["감기약 해열진통제 콧물 기침 항히스타민"]
...
def route_by_query_type(state: MedAgentState) -> str:
    ...
    drug_search → grader 통과 후 supervisor로 돌아오면 grader가 trace에 포함됨.
    grader가 있으면 검색 완료로 판단.
    """
    query_type = state.get("query_type", "simple")
    trace = state.get("agent_trace", [])

    # grader를 거쳤으면 drug_search + 평가 완료
    has_graded = "grader" in trace
