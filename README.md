...
```mermaid
flowchart TD
    U([사용자]) --> UI[Streamlit UI] --> API[FastAPI 백엔드] --> SUP

    SUP{{"Supervisor<br/>질의 분류 · 검색 키워드 추출<br/>(구어체 → 의학 용어, 분류 1회)"}}
    SUP --> DS["Drug Search<br/>Hybrid Search + Cross-Encoder Reranker"]
    DS --> GR{"Grader<br/>관련성 평가 · CRAG"}
    GR -- irrelevant --> CR["CRAG Rewrite<br/>쿼리 재작성 · 키워드 갱신"]
    CR -- "재검색 (최대 2회)" --> DS

    GR -- "단순" --> ANS
    GR -- "상호작용 / 복합" --> INT["Interaction<br/>DUR 병용금기 API Tool Use"]
    GR -- "복용주의 / 복합" --> SAF["Safety<br/>OpenSearch 금기정보 검색"]
    INT --> ANS
    SAF --> ANS

    ANS["Answer Agent<br/>GPT-4o 답변 + 인라인 출처 인용"] --> FIN([END])
    CP[("LangGraph Checkpointer · 멀티턴 대화")] -.-> SUP
